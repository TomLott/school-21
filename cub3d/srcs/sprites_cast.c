...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/06 13:42:20 by itollett          #+#    #+#             */
/*   Updated: 2021/01/06 13:42:22 by itollett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_sprite(t_all *a, t_tex *t, int j, int i)
{
	...
	{
		dst = a->img.addr + (j * a->img.line_length + i
				* (a->img.bits_per_pixel / 8));
		*(unsigned int *)dst = colour;
	}
}

void	ft_draw_sprite(t_all *a, t_tex *t, int move_scr)
{
	int d;
	int i;
	int j;
	...
	while (i < t->end_x)
	{
		t->tex_x = (int)(256 * (i - (-(t->wid) / 2
				+ t->sp_scr_x)) * t->x / t->wid) / 256;
...
void	ft_spr_cast(t_sprite *sp, t_all *a, t_tex *t)
{
	...
	while (i < a->tex.sprite)
	{
		...
		t->inv = 1.0 / (a->cam.plane_x * a->pl.dir_y
				- a->pl.dir_x * a->cam.plane_y);
		t->tr_x = t->inv * (a->pl.dir_y * t->pos_x
				- a->pl.dir_x * t->pos_y);
		t->tr_y = t->inv * (-(a->cam.plane_y) * t->pos_x
				+ t->pos_y * a->cam.plane_x);
